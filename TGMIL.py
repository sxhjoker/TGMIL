...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x1,adj):
        ...
        HSA = self.blocks(x1)
        HGA = self.GAT(torch.squeeze(x1,dim=0),adj)
        return HGA,HSA
    ...
    def forward(self, **kwargs):
        x = kwargs['data'].float()  #[1,n,1024]
        adj = kwargs['adj'].float()
        attention_only = kwargs['attention_only']
        x = torch.unsqueeze(x,dim=0)
        device = x.device
        x1 = self._fc1(x)  # [1,n,512]
        HGA,HSA = self.forward_features(x1,adj)

        HSA = torch.squeeze(HSA, dim=0)

        HSA = self.fcA(HSA)


        HSA = torch.transpose(HSA, 1, 0)  # KxN
